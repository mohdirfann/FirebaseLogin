...
public class FrontActivity extends AppCompatActivity
    ...
    List<info> dbClock= new ArrayList<>();
    ...
    private void markerForGeofence(LatLng latLng) {
        ...
        if ( map!=null ) {
            ...
            new GetAddress().execute(String.format("%.4f,%.4f",latLng.latitude,latLng.longitude));
    ...
    //xsalah aku kat sini dia wat geocoder...get address nak find uitm
    private class GetAddress extends AsyncTask<String,Void,String> {

        ProgressDialog dialog = new ProgressDialog(FrontActivity.this);
        ...
        //kat sini dia punya condition untuk clock in clock out.
        @Override
        protected void onPostExecute(String s) {
            try{
                ...
                Toast.makeText(getApplicationContext(),String.valueOf(contains),Toast.LENGTH_SHORT).show();
                            ...
                            myInfo.addValueEventListener(new ValueEventListener() {
                                ...
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    ...
                                    for(DataSnapshot child:children){
                                        ...
                                        info temp = child.getValue(info.class);
                                        dbClock.add(temp);
                                    }
                                    Log.d("Testing4","ArrayList");
                                    for(int i=0;i<dbClock.size();i++){
                                        Log.d("Testing5","dbClock");
                                        info tempList = dbClock.get(i);
                                        if(tempList.getDateIn().equalsIgnoreCase("-")){
                                        String[] tempClkIn = tempList.getLate().split(",");
                                        Log.d("Testing6",String.valueOf(tempClkIn[0]));
                                        if(tempList.getName().equalsIgnoreCase(id)&&tempClkIn[0].equalsIgnoreCase(splitCurrDate[0])){
                                            info updtClock = new info(tempList.getName(),tempList.getDateIn(), tempList.getLate() , tempList.getAddress(), currDate,tempList.getKey());
                                            myInfo.child(tempList.getKey()).setValue(updtClock);
                                            Log.d("ClockOu",updtClock.getName());
                                            clockOutBtn.setEnabled(false);
                                        }
                                        }
                                        else{
                                            String[] tempClkIn = tempList.getDateIn().split(",");
                                            Log.d("Testing6",String.valueOf(tempClkIn[0]));
                                            if(tempList.getName().equalsIgnoreCase(id)&&tempClkIn[0].equalsIgnoreCase(splitCurrDate[0])){
                                                info updtClock = new info(tempList.getName(),tempList.getDateIn(), tempList.getLate() , tempList.getAddress(), currDate,tempList.getKey());
                                                myInfo.child(tempList.getKey()).setValue(updtClock);
